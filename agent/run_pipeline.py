...
"""
    try:
        ...
        with urllib.request.urlopen(req, timeout=30) as response:
            ...
            return int(parsed.get("score", 50)), parsed.get("reason", "Evaluated by Ollama")
...
def send_telegram_alert(title, company, location, url, score, reason):
    ...
    clean_company = html.escape(str(company))
    clean_location = html.escape(str(location))
...
def process_pipeline():
    ...
    logging.info(f"Found {len(jobs)} unprocessed jobs with status='APPLY'")
    ...
    for job in jobs:
        ...
        logging.info(f"Ollama Evaluation -> Score: {score} | Reason: {reason}")
